...
namespace OpenMps
{
	...
	private:
		...
		// 圧力勾配を計算する関数ポインタの型
		typedef Vector(Particle::*GetPressureGradientFunc)(const Particle::List& particles, const double r_e, const double dt, const double rho, const double n0) const;
	...
	public:
		...
		// @param particles 粒子リスト
		// @param r_e 影響半径
		void UpdateNeighborDensity(const Particle::List& particles, const double r_e)
		{
			return (this->*(Particle::UpdateNeighborDensityFuncTable[type]))(particles, r_e);
		}
		...
		Vector GetCorrectionByTooNear(const Particle::List& particles, const double r_e, const double rho, const double tooNearRatio, const double tooNearCoefficient) const
		{
			return (this->*(Particle::GetCorrectionByTooNearFuncTable[type]))(particles, r_e, rho, tooNearRatio, tooNearCoefficient);
		}
#endif
